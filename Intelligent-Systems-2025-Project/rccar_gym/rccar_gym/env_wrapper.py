'''
    This RCCarWrapper is for 
        1. usage issue (gymnasium style)
        2. define observation & action space
        3. observation processing (concatenate lidar + other properties)
        
        ->  outputs of step function should have (n_agents, dim) shape
            ex) observations, rewards, terminates, truncates, infos = env.step([agent_1_action, 
                                                                                agent_2_action,
                                                                                ,,,
                                                                                agent_n_action])
'''

import numpy as np
import gymnasium as gym
import rccar_gym

class RCCarWrapper(gym.Wrapper):
    def __init__(self, args=None, maps=None, render_mode=None, seed=None) -> None:
        self._env = gym.make("rccar-v0", 
                            args=args,
                            maps=maps,
                            render_mode=render_mode,
                            seed=seed)
        super().__init__(self._env)
        np.random.seed(self._env.unwrapped.seed)
        
        start, end = (np.array(args.lidar_range) * 4 + 540).astype(int)      # degree to scan index
        self.start, self.end = max(0, start), min(1080, end)

        #####
        self.num_agents = self._env.unwrapped.num_agents
        self.num_controlled_agents = self._env.unwrapped.num_controlled_agents
        self.track = self._env.unwrapped.track
        self.waypoints = np.stack([self.track.centerline.xs, self.track.centerline.ys]).T
        self.N = self.waypoints.shape[0]
        self.checkpoints = [[] for _ in range(self.num_controlled_agents)]
        self.n_checkpoints = 20
        self.next_checkpoints = np.zeros(self.num_controlled_agents, dtype=int)
        self.arrive_distance = 1.5
        #####
        
        self.max_speed = args.max_speed
        self.min_speed = args.min_speed
        self.max_steer = args.max_steer

    def reset(self, **kwargs):
        obs_dict, info = self._env.reset(**kwargs)

        pos_x, pos_y, yaw = obs_dict['poses_x'], obs_dict['poses_y'], obs_dict['poses_theta']
        self.pos = np.stack([pos_x, pos_y], axis=1)
        self.yaw = yaw
        self.scan = obs_dict['scans'][0, self.start:self.end]

        for i in range(self.num_controlled_agents):
            agent_pos = self.pos[i]
            dpos = self.waypoints - agent_pos
            dists = np.linalg.norm(dpos, axis=-1)
            init_idx = np.argmin(dists)
            for j in range(self.n_checkpoints):
                self.checkpoints[i].append(self.waypoints[(int(self.N*(j+1)/self.n_checkpoints)-1+init_idx)%self.N])

        return [self.pos, yaw, self.scan], info

    def step(self, action:np.array):
        steer = action[:self.num_controlled_agents, 0]
        speed = action[:self.num_controlled_agents, 1]

        steer = np.clip(steer, -self.max_steer, self.max_steer)
        speed = np.clip(speed, self.min_speed, self.max_speed)
        
        clipped_actions = np.zeros((self.num_agents, 2))
        clipped_actions[:self.num_controlled_agents, 0] = steer
        clipped_actions[:self.num_controlled_agents, 1] = speed

        obs_dict, _, terminate, truncate, info = self._env.step(clipped_actions)

        pos_x, pos_y, yaw = obs_dict['poses_x'], obs_dict['poses_y'], obs_dict['poses_theta']
        self.pos = np.stack([pos_x, pos_y], axis=1)
        self.yaw = yaw
        self.scan = obs_dict['scans'][0, self.start:self.end]

        info['obs_dict'] = obs_dict
        reward = 0.0

        #####
        running_agent_idx = np.where(self.next_checkpoints != self.n_checkpoints)[0]
        running_checkpoint_pos = np.array(self.checkpoints)[running_agent_idx, self.next_checkpoints]
        arrived_idx = running_agent_idx[np.linalg.norm(self.pos - running_checkpoint_pos, axis=-1) < self.arrive_distance]
        self.next_checkpoints[arrived_idx] += 1
        info['waypoint'] = int(self.next_checkpoints[0])
        #####
        return [self.pos, self.yaw, self.scan], reward, terminate, truncate, info
    
    def render(self):
        return self._env.render()

    def close(self):
        self._env.close()
