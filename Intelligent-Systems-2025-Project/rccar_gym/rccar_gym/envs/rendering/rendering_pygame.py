...
INSTRUCTION_LEFT = "Mouse click: Change POV"
INSTRUCTION_RIGHT = "'S' key: On/Off"
...
class PygameEnvRenderer(EnvRenderer):
    ...
    def __init__(
        ...
        self.top_left_info_renderer = TextObject(
            window_shape=(width, height), position="top_left"
        )
        self.top_right_info_renderer = TextObject(
            window_shape=(width, height), position="top_right"
        )
    ...
    def render(self) -> Optional[np.ndarray]:
        ...
        if self.draw_flag:
            ...
            self.top_right_info_renderer.render(
                texts=[f"Velocity: {self.cars[0].velocity:.2f}", 
                       f"Steering: {self.cars[0].steering:.2f}"], 
                display=self.canvas
            )
            self.top_left_info_renderer.render(
                texts=[f"action[0]: {self.cars[0].action[0]:.2f}", 
                       f"action[1]: {self.cars[0].action[1]:.2f}"], 
                display=self.canvas
            )

        if self.render_spec.show_info:
            self.bottom_info_renderer.render(texts=[INSTRUCTION_LEFT], display=self.canvas)
            self.top_info_renderer.render(texts=[self.track.name], display=self.canvas)
        self.time_renderer.render(texts=[f"{self.sim_time:.2f}"], display=self.canvas)

        if self.render_mode in ["human", "human_fast"]:
            assert self.window is not None

            self.fps_renderer.render(
                texts=[f"FPS: {self.clock.get_fps():.2f}"], display=self.canvas
            )
