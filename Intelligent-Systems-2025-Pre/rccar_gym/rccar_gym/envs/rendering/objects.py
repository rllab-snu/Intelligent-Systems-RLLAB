...
class TextObject:
    ...
    def __init__(
        ...
        text = self.font.render("", True, (125, 125, 125))

    def _position_resolver(
        self, text, position: str | tuple[int, int], display: pygame.Surface
    ) -> tuple[int, int]:
        """
    ...
    def render(self, texts: str, display: pygame.Surface) -> None:
        """
        ...
        for i, text in enumerate(texts):
            text = self.font.render(text, True, (125, 125, 125))
            height = text.get_height()
            position_tuple = self._position_resolver(text, self.position, display)
            position_tuple[1] += height*i
            display.blit(text, position_tuple)
