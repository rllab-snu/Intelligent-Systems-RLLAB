...
from .utils import find_track_dir, nearest_point_on_trajectory
...
class Track:
    ...
    # frenet conversion without guess
    def cartesian_to_frenet2(self, x, y, phi):
        """
        Convert Cartesian coordinates to Frenet coordinates.
        ...
        returns:
            ...
            ephi: heading deviation
        """
        _, _, _, min_dist_segment = nearest_point_on_trajectory(
            np.array([x, y]), self.centerline.spline.points
        )
        s, ey = self.centerline.spline.calc_arclength(x, y, self.centerline.spline.s[min_dist_segment])
        if s > self.centerline.spline.s[-1]:
            # Wrap around
            s = s - self.centerline.spline.s[-1]
        if s < 0:
